...
const QUEST_ITEM : PackedScene = preload("res://GUI/Quests/quest_item.tscn")
const QUEST_STEP_ITEM : PackedScene = preload("res://GUI/Quests/quest_step_item.tscn")
...
func _on_visible_changed():
	...
	if visible == true:
		...
		for q in QuestManager.current_quests:
			...
			var new_quest_item : QuestItem = QUEST_ITEM.instantiate()
			quest_item_container.add_child( new_quest_item )
			new_quest_item.initialize( quest_data, q )
			new_quest_item.focus_entered.connect( update_quest_details.bind( new_quest_item.quest ) )
	pass
...
func update_quest_details( q : Quest ):
		...
		for step in q.steps:
			...
			var step_is_completed : bool = false
			if quest_save.title != "not found":
				step_is_completed = quest_save.completed_steps.has( step.to_lower() )
			details_container.add_child( new_step )
			new_step.initialize( step, step_is_completed )
		pass
